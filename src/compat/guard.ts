...
export const SUPPORTED_ASSETS: readonly AssetRecord[] = getBuildProfile("8dd50222").assets;
...
class DefaultCompatibilityProbe implements CompatibilityProbe {
  constructor(private readonly profile = getBuildProfile("8dd50222")) {}

  async inspect(assets: ReadonlyMap<string, Uint8Array>): Promise<CompatibilityInspection> {
    const sources = new Map<string, string>();
    const decoder = new TextDecoder("utf-8", { fatal: true });
    for (const record of this.profile.assets.filter(({ kind }) => kind === "javascript")) {
    ...
    const modules = inspectOfficialWorkerContract(sources, this.profile.officialWorker);
    const wasmRecord = this.profile.assets.find(({ kind }) => kind === "wasm")!;
...
function assertManifest(session: SessionExport, expectedAssets: readonly AssetRecord[]): void {
...
export class CompatibilityGuard {
  constructor(
    ...
    private readonly probe: CompatibilityProbe | undefined = undefined,
    private readonly profile = getBuildProfile("8dd50222"),
  ) {}

  async verify(session: SessionExport): Promise<CompatibilityReport> {
    if (session.buildId !== this.profile.buildId) {
      throw new AppError("UNSUPPORTED_BUILD", "Unsupported Snapchat Web build", {
        buildId: session.buildId,
      });
    }
    assertManifest(session, this.profile.assets);
    const loaded = new Map<string, Uint8Array>();
    for (const record of this.profile.assets) {
      loaded.set(record.filename, await this.loader.loadVerified(record));
    }
    let inspection: CompatibilityInspection;
    try {
      inspection = await (this.probe ?? new DefaultCompatibilityProbe(this.profile)).inspect(loaded);
    } catch (error) {
    ...
    return {
      buildId: this.profile.buildId,
      assets: this.profile.assets.map(({ filename, sha256, size }) => ({ filename, sha256, size })),
